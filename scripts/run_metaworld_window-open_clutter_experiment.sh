...
# Metaworld Window-open clutter experiment: Compare AEDP3 and DP3 generalization ability
# with 8 distraction objects in window-open environment
# Experiment design:
# - AEDP3: with attention mechanism (attn mode)
# - DP3: without attention mechanism (no_attn mode)
# - Environment: 8 distraction blocks around the window area
#
...
log() { echo -e "[window-open_clutter] $*"; }

# Run AEDP3 clutter experiment
run_aedp3_clutter() {
    log "=== Running AEDP3 clutter experiment ==="
    task_name="metaworld_window-open_clutter"
...
run_dp3_clutter() {
    ...
    task_name="metaworld_window-open_no_attn_clutter"
...
# Main function
main() {
    log "Metaworld Window-open clutter experiment started"
    log "Configuration:"
    log "  ROOT=${ROOT}"
    log "  GPU_ID=${GPU_ID}"
    log "  SEED=${SEED}"
    ...
    log "  - AEDP3 clutter: data/outputs/metaworld_window-open_clutter-${CONFIG_NAME}-AEDP3_clutter_seed${SEED}"
    log "  - DP3 clutter: data/outputs/metaworld_window-open_no_attn_clutter-${CONFIG_NAME}-DP3_clutter_seed${SEED}"
