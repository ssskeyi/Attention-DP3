...
# 可视化DexArt GS2分割结果
# 将结果保存在项目根目录的vis_res_dexart文件夹下

ROOT="${ROOT:-$(cd "$(dirname "$0")/.."; pwd)}"
VIS_ROOT="${VIS_ROOT:-${ROOT}/vis_res_dexart}"

# 实验配置（与make_dexart_datasets.sh保持一致）
TASKS="${TASKS:-bucket faucet laptop toilet}"
MAX_EP="${MAX_EP:-10}"

log() { echo -e "[visualize_dexart] $*"; }
...
# 可视化GS2分割结果
visualize_gs2_seg() {
    log "=== 开始可视化DexArt GS2分割结果 ==="

    for task in ${TASKS}; do
        log "可视化 ${task} 的GS2分割..."

        gs2_root="${ROOT}/3D-Diffusion-Policy/export_gs2/dexart_${task}"
        frames_root="${ROOT}/3D-Diffusion-Policy/export/dexart_${task}_frames"
