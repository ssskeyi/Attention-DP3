...
# Metaworld Stick-push clutter experiment: Compare AEDP3 and DP3 generalization ability
# with 8 distraction objects in stick-push environment
# Experiment design:
# - AEDP3: with attention mechanism (attn mode)
# - DP3: without attention mechanism (no_attn mode)
# - Environment: 8 distraction blocks around the stick-push area
#
...
# Main function
main() {
    log "Metaworld Stick-push clutter experiment started"
    log "Configuration:"
    log "  ROOT=${ROOT}"
    log "  GPU_ID=${GPU_ID}"
    log "  SEED=${SEED}"
